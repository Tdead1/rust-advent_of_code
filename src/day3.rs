...
{	
	...
	let symbols_with_indices = lines
	.iter()
	.map(|i| i
		.chars()
		.enumerate()
		.filter(|i| i.1 != '.' && !i.1.is_ascii_digit())
		.collect::<Vec<(usize, char)>>())
		.collect::<Vec<Vec<(usize, char)>>>();
	
	let numbers_with_indices = lines
	.iter()
	.map(|i| i
		.chars()
		.enumerate()
		.filter(|i| i.1.is_ascii_digit())
		.collect::<Vec<(usize, char)>>())
		.collect::<Vec<Vec<(usize, char)>>>();

	// making a new list with the values of the numbers (still indexed).
	let mut values_with_indices : Vec<Vec<(usize, u32)>> = numbers_with_indices
	.clone()
	.iter()
	.map(|i| i
		.iter()
		.map(|i| (i.0, i.1.to_digit(10).unwrap_or_default() as u32))
		.collect::<Vec<(usize, u32)>>())
	.collect::<Vec<Vec<(usize, u32)>>>();

	// Some arcane magic to modify the string so the numbers are all the "full numbers" as humanly readable.
	for i in 0..numbers_with_indices.len()
	{
		if numbers_with_indices[i].len() == 0 { continue; }
		let line = &numbers_with_indices[i];
		
		let mut modified_numbers = 1; 
		for j in 1 .. numbers_with_indices[i].len()
		{
			if line[j].0 == line[j - 1].0 + 1
			{
				values_with_indices[i][j].1 = line[j].1.to_digit(10).unwrap_or_default() as u32 + values_with_indices[i][j - 1].1 * 10 as u32;
				modified_numbers = modified_numbers + 1;
				for pervious_indices in 0..modified_numbers
				{
					values_with_indices[i][j - pervious_indices].1 = values_with_indices[i][j].1;
				}
			}
			else 
			{
	...
	// store all numbers that are relevant into a new array (format line, index, size)
	let mut relevant_numbers : Vec<(usize, usize, u32)> = Vec::new();
	for line_index in 0..values_with_indices.len()
	{
		let values_on_line = values_with_indices.get(line_index as usize);

		let symbols_on_line_below = symbols_with_indices.get((line_index + 1) as usize);
		let symbols_on_line_middle = symbols_with_indices.get((line_index) as usize);
		let symbols_on_line_above = if line_index != 0 { symbols_with_indices.get(line_index - 1) } else { None };
		let search_lines = [symbols_on_line_below, symbols_on_line_middle, symbols_on_line_above];

		let mut most_recent_index_added = None;
		for indexed_value in match values_on_line { Some(_) => values_on_line.unwrap(), None => continue, }
		{
			for search_line in search_lines
			{
				match search_line
				{
					Some(_) => 
					{
						let symbol_left   = search_line.unwrap().iter().find(|i| i.0 as i32 == (indexed_value.0 as i32) - 1);
						let symbol_middle = search_line.unwrap().iter().find(|i| i.0 == indexed_value.0);
						let symbol_right  = search_line.unwrap().iter().find(|i| i.0 == indexed_value.0 + 1);
						match symbol_left
						{
							Some(_) => symbol_left.unwrap(),
							None => match symbol_right
							{
								Some(_) => symbol_right.unwrap(),
								None => match symbol_middle
								{
									Some(_) => symbol_middle.unwrap(),
									None => continue,
								}
							}
						};
			
						most_recent_index_added = relevant_numbers.last().take();
						let should_add_value = match most_recent_index_added
						{
							Some(_) => 
							most_recent_index_added.unwrap().0 != line_index 
							|| most_recent_index_added.unwrap().1 != indexed_value.0 - 1
							|| most_recent_index_added.unwrap().2 != indexed_value.1,
							None => true,
						};
						if should_add_value
						{
							relevant_numbers.push((line_index, indexed_value.0, indexed_value.1));
						}
						else 
						{
							let last_index = relevant_numbers.len() - 1;
							relevant_numbers[last_index] = (line_index, indexed_value.0, indexed_value.1);
						}
	...
	// Sum it up.
	println!("Rows: {} Columns: {}", symbols_with_indices.len(), symbols_with_indices[0].len());
	println!("Answer = {} ", relevant_numbers.iter().map(|i| i.2).sum::<u32>())

	// Debug:
	//for i in symbols_with_indices
	//{
	//	for j in i
	//	{
	//		print!("{}[{}]", j.1, j.0);
