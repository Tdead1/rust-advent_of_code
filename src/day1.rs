...
fn version_two() {
    // This needs to go from left to right, instead of low number to high number.
    let mut text : String = INPUT.to_string();
    for number in NUMBERS
    {
        text = text.replace(number.0, number.1);
	}
    ...
    .map(|line| line
        .chars()
        .filter_map(|i| match i.is_numeric() { true => Some(i), false => None })
        .collect())
    .map(|i : Vec<char>| (i.first().unwrap_or_else(|| &'0').to_digit(10).unwrap_or_else(||0) * 10) + i.last().unwrap_or_else(|| &'0').to_digit(10).unwrap_or_else(||0))
    .sum();
	...
	/* Debug version: */
	/*
    let numbers_from_lines : Vec<Vec<char>>
    = text
    .as_str()
    .lines()
    .map(|line| line
    ...
    let total : u32 = (&numbers_from_lines)
    .into_iter()
    .map(|i| (i.first().unwrap_or_else(|| &'0').to_digit(10).unwrap_or_else(||0) * 10) + i.last().unwrap_or_else(|| &'0').to_digit(10).unwrap_or_else(||0))
    .sum();

    for i in numbers_from_lines
    {
        print!("{:?} + {:?} = ", i.first().unwrap_or_else(|| &'0').to_digit(10).unwrap_or_else(||0), i.last().unwrap_or_else(|| &'0').to_digit(10).unwrap_or_else(||0));
        println!("{:?}", i);
    }
